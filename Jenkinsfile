...
pipeline {
    ...
    stages {
        ...
        stage("Helm dry-run") {
            steps {
                sh "helm upgrade --dry-run --wait --install ${params.helm_chart} ${params.helm_chart} --set image.tag=${params.image_tag}"
            }
        }
        stage("Helm deploy") {
            when {
            ...
            steps {
                ...
                sh "helm upgrade --wait --install ${params.helm_chart} ${params.helm_chart} --set image.tag=${params.image_tag}"
            }
        }
    }
    post {
